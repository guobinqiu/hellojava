        ...
        stage('Deploy with Ansible') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: '54e6cd1d-d917-4427-ac13-6d7ff0abdc39', keyFileVariable: 'SSH_KEY')]) {
                    //sh 'ansible-playbook -u guobin -e "artifact=/var/lib/jenkins/workspace/hellojava/target/hellojava-${GIT_COMMIT}.war,ansible_ssh_private_key_file=${SSH_KEY}" /etc/ansible/hellojava.yml'
